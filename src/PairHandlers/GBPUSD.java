...
public class GBPUSD extends CommonHandlerForPair{
    public GBPUSD(DatabaseConnection dbc){
        this.dbc=dbc;
    ...
    public String getClosePrice(String FromDate) throws SQLException{
        if(dbc!=null){
            ResultSet rs = dbc.getData("SELECT close_price FROM HISTORICAL_DATA.GBP_USD WHERE DAY_DATE=\'"+FromDate+"\';");
            if(rs!=null && rs.next())
    ...
    public ArrayList<Float> getClosePrice(String FromDate,String ToDate)throws SQLException{
        if(dbc!=null){
            ...
            ResultSet rs = dbc.getData("SELECT close_price FROM HISTORICAL_DATA.GBP_USD WHERE DAY_DATE BETWEEN "+FromDate2+" AND "+FromDate+";");
    ...
    public ArrayList<ArrayList<Object>> getAllData(){
        ...
        ResultSet rs = this.dbc.getData("SELECT DAY_DATE,close_price from historical_data.gbp_usd");
        try {
